...
<code>
  ...
  <template id="left" data-path="'left'" data-active="edit"></template>
    ...
    <div class="panel-heading"><h4 class="panel-title">修改模板</h4></div>
   <div class="panel-body">
    <form name="form" action="/aeolus/sms/save" method="post" data="form_data" class="form" ng-submit="save(form)">
      <input type="hidden" name="id" ng-value="form.$$data.__id">
      <div class="form-group">
        <label class="control-label">模板名*</label>
        <input type="text" name="name" required="" ng-maxlength=20 maxlength="20" class="form-control" ng-model="form.$$data.name">
      </div>
      <div class="form-group">
...
function($this, $http, $location, $layer){
  var load = $layer.load();
  $http.get('/aeolus/sms/detail/id/'+$location.$$search.id).success(function(data){
    load.close();
    if(200==data.code){
      $this.form_data = data.data;
    }else{
      $layer.msg(data.msg);
      $location.path('/sms/index').search({});
    }
