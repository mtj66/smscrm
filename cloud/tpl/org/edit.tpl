...
<code>
  ...
  <template id="left" data-path="'left'" data-active="edit"></template>
  <div id="right">
  <div class="panel panel-default">
    <div class="panel-heading"><h4 class="panel-title">修改客户</h4></div>
   <div class="panel-body">
    <form name="form" action="/aeolus/org/save" method="post" class="form" data="form_data" ng-submit="save(form)">
      <input type="hidden" name="id" ng-value="form.$$data.org_id">

      <div class="form-group">
        <label class="control-label control-required">客户名称</label>
        <span ng-bind="form.$$data.org_name"></span>
      </div>
      <div class="form-group">
        <label class="control-label control-required">帐号</label>
        <input type="text" name="mobile" required="" ng-model="form.$$data.mobile" maxlength="11" class="form-control form-control-inline" ng-disabled="!form.$$data.rebind>0">
      </div>
      <div class="form-group">
        <label class="control-label control-required">密码</label>
        <input type="password" class="form-control form-control-inline" required="" name="password" ng-model="form.$$data.password" ng-disabled="!form.$$data.rebind>0">
      </div>
      <div class="form-group">
        <label class="control-label">修改帐号/密码</label>
        <label>
        <input type="checkbox" name="rebind" ng-value=1 ng-model="form.$$data.rebind"></label>
        <label>
      </div>
      <div class="form-group">
...
var load=$layer.load(3);
  $http.get('/aeolus/org/detail/id/%s'.sprintf($location.$$search.id||0)).success(function(data){
    load.close()
    angular.extend($this.form_data, data.data);
    $this.form_data.kpi_score=parseFloat($this.form_data.kpi_score)
    $this.form_data.price=parseFloat($this.form_data.price)
    $this.form_data.fee=parseFloat($this.form_data.fee)
  })
